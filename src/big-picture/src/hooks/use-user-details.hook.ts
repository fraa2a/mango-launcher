...
export function useUserDetails() {
  ...
  const unblockUser = useCallback(async (userId: string) => {
    return globalThis.window.electron.mangoApi.post(`/users/${userId}/unblock`);
  }, []);

  const sendFriendRequest = useCallback(async (userId: string) => {
    return globalThis.window.electron.mangoApi.post(
  ...
  const updateFriendRequestState = useCallback(
    async (userId: string, action: FriendRequestAction) => {
      if (action === "CANCEL") {
        return globalThis.window.electron.mangoApi.delete(
  ...
  const blockUser = useCallback(async (userId: string) => {
    return globalThis.window.electron.mangoApi.post(`/users/${userId}/block`);
  }, []);
