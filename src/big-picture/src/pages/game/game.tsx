...
export default function Game() {
  ...
  const { launchSettings, customizationSettings } = useGameSettingsModalState({
    game,
      ...
      try {
        ...
        if (code === "PLATFORM_UNKNOWN") {
          showErrorToast("Platform not supported", {
            message: "Mango could not identify an emulator for this platform.",
          });
          return;
        }
        ...
        if (code === "PKG_UNREADABLE") {
          showErrorToast("Unsupported PKG", {
            message:
              "Mango could not read this package. Install and launch it from RPCS3 directly.",
          });
          return;
        }
        ...
        showErrorToast("Launch failed", {
          message: "Mango could not launch this Classics game.",
        });
      }
    },
    ...
    <VerticalFocusGroup regionId={GAME_PAGE_REGION_ID} asChild>
      <div ref={pageRef} className="game-page">
        ...
        <ConfirmationModal
          ...
          description="Close the current RPCS3 session before launching this game, or force Mango to start it again."
