...
function isMangoNotification(notification: MergedNotification) {
  return (
...
export function SidebarNotificationsDropdown({
  ...
  const fetchApiNotifications = useCallback(async () => {
    ...
    try {
      const [notificationsResponse, countResponse] = await Promise.all([
        globalThis.window.electron.mangoApi.get<NotificationsResponse>(
        ...
        globalThis.window.electron.mangoApi.get<NotificationCountResponse>(
  ...
  const markAsRead = async (notification: MergedNotification) => {
    if (notification.source === "local") {
      ...
      await globalThis.window.electron.mangoApi.patch(
  ...
  const dismissNotification = async (notification: MergedNotification) => {
    if (notification.source === "local") {
      ...
      await globalThis.window.electron.mangoApi.delete(
  ...
  const updateFriendRequest = (
    ...
    return globalThis.window.electron.mangoApi.patch(
  ...
  const renderNotificationIcon = (notification: MergedNotification) => {
    ...
    if (isMangoNotification(notification)) {
      return (
        <div className="sidebar-notifications-dropdown__fallback-icon">
          <img
            src={mangoIconUrl}
