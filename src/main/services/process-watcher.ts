...
import { createGame, trackGamePlaytime } from "./library-sync-stubs";
...
const hasLinuxCompatibilityProcessMatch = (
  ...
  const steamCompatRoot = path
    .join(app.getPath("userData"), "steam-compat", game.objectId)
    .toLowerCase();
  const expectedCompatPaths = [expectedWinePrefix, steamCompatRoot].filter(
    Boolean
  ) as string[];
  ...
  return linuxProcesses.some((process) => {
    ...
    if (
      expectedCompatPaths.length > 0 &&
      process.steamCompatDataPath &&
      !expectedCompatPaths.includes(process.steamCompatDataPath)
    ) {
      return false;
    }
...
function onOpenGame(game: Game) {
  ...
  // Hide Mango to tray on game startup if enabled
