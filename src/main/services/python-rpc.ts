...
const binaryNameByPlatform: Partial<Record<NodeJS.Platform, string>> = {
  darwin: "mango-python-rpc",
  linux: "mango-python-rpc",
  win32: "mango-python-rpc.exe",
};
...
export class PythonRPC {
  ...
  public static async spawn(
    ...
    if (app.isPackaged) {
      ...
      if (!fs.existsSync(binaryPath)) {
        dialog.showErrorBox(
          "Fatal",
          "Mango Python Instance binary not found. Please check if it has been removed by Windows Defender."
        );

        app.quit();
        throw new Error(`Mango Python RPC binary not found at ${binaryPath}`);
      }
  ...
  private static resolvePythonExecutable() {
    ...
    throw new Error(
      "Python executable not found. Set MANGO_PYTHON_BIN or install python3/python."
    );
  }
