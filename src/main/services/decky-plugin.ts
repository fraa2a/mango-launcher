...
export class DeckyPlugin {
  ...
  private static async getDeckyReleaseInfo(): Promise<DeckyReleaseInfo> {
    ...
    try {
      const response = await MangoApi.get<DeckyReleaseInfo>(
        "/decky/release",
        {},
        { needsAuth: false }
      );
  ...
  private static getPackageJsonPath(): string {
    return path.join(MANGO_DECKY_PLUGIN_LOCATION, "package.json");
  }

  private static async downloadPlugin(): Promise<string> {
    logger.log("Downloading Mango Decky plugin...");

    const releaseInfo = await this.getDeckyReleaseInfo();
    const tempDir = SystemPath.getPath("temp");
    const zipPath = path.join(tempDir, "Mango.zip");
  ...
  private static async extractPlugin(zipPath: string): Promise<string> {
    logger.log("Extracting Mango Decky plugin...");

    const tempDir = SystemPath.getPath("temp");
    const extractPath = path.join(tempDir, "mango-decky-plugin");
  ...
  private static async installPluginWithSudo(
    ...
    return new Promise((resolve, reject) => {
      const command = `mkdir -p "${DECKY_PLUGINS_LOCATION}" && rm -rf "${MANGO_DECKY_PLUGIN_LOCATION}" && cp -r "${sourcePath}" "${MANGO_DECKY_PLUGIN_LOCATION}" && chown -R ${username}: "${DECKY_PLUGINS_LOCATION}"`;
  ...
  private static async installPluginWithoutSudo(
    ...
    if (fs.existsSync(MANGO_DECKY_PLUGIN_LOCATION)) {
      await fs.promises.rm(MANGO_DECKY_PLUGIN_LOCATION, {
        recursive: true,
        force: true,
      });
    }

    await fs.promises.cp(sourcePath, MANGO_DECKY_PLUGIN_LOCATION, {
      recursive: true,
    });
  ...
  public static async checkAndUpdateIfOutdated(): Promise<void> {
    if (!fs.existsSync(MANGO_DECKY_PLUGIN_LOCATION)) {
      logger.log("Mango Decky plugin not installed, skipping update check");
      return;
    }
    ...
    try {
      if (!fs.existsSync(packageJsonPath)) {
        logger.log(
          "Mango Decky plugin package.json not found, skipping update"
        );
        return;
      }
      ...
      if (isOutdated) {
        logger.log(
          `Mango Decky plugin is outdated. Current: ${currentVersion}, Expected: ${releaseInfo.version}. Updating...`
        );

        await this.updatePlugin();
        logger.log("Mango Decky plugin updated successfully");
      } else {
        logger.log(`Mango Decky plugin is up to date (${currentVersion})`);
      }
    } catch (error) {
      logger.error(`Error checking/updating Mango Decky plugin: ${error}`);
    }
  }
  ...
  public static async checkPluginVersion(): Promise<{
    ...
    try {
      ...
      if (!fs.existsSync(MANGO_DECKY_PLUGIN_LOCATION)) {
        logger.log("Mango Decky plugin folder not found, installing...");
      ...
      try {
        if (!fs.existsSync(packageJsonPath)) {
          logger.log(
            "Mango Decky plugin package.json not found, installing..."
          );
        ...
        if (isOutdated) {
          logger.log(
            `Mango Decky plugin is outdated. Current: ${currentVersion}, Expected: ${releaseInfo.version}`
          );
        ...
        logger.error(`Error checking Mango Decky plugin version: ${error}`);
        return {
          exists: false,
          outdated: true,
          currentVersion: null,
