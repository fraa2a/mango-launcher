...
export class Umu {
  ...
  public static async findBestProtonPath(): Promise<string | null> {
    const versions = await Umu.getInstalledProtonVersions();
    if (versions.length === 0) return null;

    const geProton = versions.find((v) => v.name.startsWith("GE-Proton"));
    if (geProton) return geProton.path;
