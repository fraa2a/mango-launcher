...
export class NativeAddon {
  private static nativeModule: MangoNativeModule | null = null;
  ...
  private static resolveAddonPath() {
    ...
    return path.join(app.getAppPath(), "mango-native", "mango-native.node");
  }
  ...
  private static load() {
    ...
    if (!fs.existsSync(addonPath)) {
      throw new Error(`Mango native addon not found at ${addonPath}`);
    }
    ...
    const nativeModule = require(addonPath) as MangoNativeModule;
  ...
  public static processProfileImage(
    ...
    try {
      ...
      if (!normalizedImagePath || !normalizedMimeType) {
        throw new Error("Mango native addon returned an invalid payload");
      }
