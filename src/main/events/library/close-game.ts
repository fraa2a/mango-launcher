...
const killWineServerInPrefix = async (prefixRoot: string) => {
  const wineserverBin = path.join(prefixRoot, "pfx");
  if (!fs.existsSync(path.join(wineserverBin, "dosdevices"))) return;

  try {
    const { spawnSync } = await import("node:child_process");
    spawnSync("wineserver", ["-k"], {
      env: { ...process.env, WINEPREFIX: wineserverBin },
      stdio: "ignore",
    });
  } catch {
    // best-effort
...
const closeGame = async (
  ...
  const steamCompatRoot = path.join(
    app.getPath("userData"),
    "steam-compat",
    objectId
  );
  if (fs.existsSync(path.join(steamCompatRoot, "pfx"))) {
    logger.info("Killing wineserver in steam-compat prefix", {
      steamCompatRoot,
    });
    await killWineServerInPrefix(steamCompatRoot);
  }
};
