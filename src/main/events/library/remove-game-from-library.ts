...
import { MangoApi, logger } from "@main/services";
import {
  gamesSublevel,
...
const removeGameFromLibrary = async (
  ...
  if (game.remoteId) {
    MangoApi.delete(`/profile/games/${game.remoteId}`).catch(() => {});
  }

  await deleteAssetFiles(assetPathsToDelete);

  const steamCompatPath = path.join(
  ...
  if (fs.existsSync(steamCompatPath)) {
    await fs.promises
      ...
      .catch(() => {
        logger.warn("Failed to clean up steam-compat directory", {
          steamCompatPath,
        });
      });
  }
