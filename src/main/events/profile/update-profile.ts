...
import { MangoApi, WindowManager } from "@main/services";
...
export const patchUserProfile = async (updateProfile: UpdateProfileRequest) => {
  return MangoApi.patch<UserProfile>("/profile", updateProfile);
};
...
const uploadImage = async (
  ...
  const response = await MangoApi.post<{ presignedUrl: string }>(
    `/presigned-urls/${type}`,
    {
...
const updateProfile = async (
  ...
  // Notify every app window so they can re-fetch the signed-in user's
  // details after a profile change.
