...
import {
  ...
  importMangoCustomDownloadSource,
} from "@main/services";
...
import { loadCriticalState, runDeferredStartup } from "./main";
...
const PROTOCOL = "mangolauncher";
const SUPPORTED_PROTOCOLS = [PROTOCOL] as const;
const MANGO_CUSTOM_DOWNLOAD_SOURCE_EXTENSION = ".mangocds";

let pendingCustomDownloadSourceFile: string | null = null;

const findDeepLink = (args: string[]) =>
  args.find((arg) =>
    SUPPORTED_PROTOCOLS.some((scheme) => arg.startsWith(`${scheme}://`))
  );

const findCustomDownloadSourceFile = (args: string[]) =>
  args.find((arg) =>
    arg.toLowerCase().endsWith(MANGO_CUSTOM_DOWNLOAD_SOURCE_EXTENSION)
  );
...
const initializeApp = async () => {
  electronApp.setAppUserModelId("app.mangolauncher.mango");
  ...
  const deepLinkArg = findDeepLink(process.argv);
  const customSourceFile =
    findCustomDownloadSourceFile(process.argv) ??
    pendingCustomDownloadSourceFile;
  const forceBigPicture = process.argv.includes("--big-picture");
  const isRunDeepLink = deepLinkArg
    ? new URL(deepLinkArg).host === "run"
    : false;
  ...
  if (customSourceFile) {
    void handleCustomDownloadSourceFile(customSourceFile);
  }

  // Deferred startup runs after the window is created so it never blocks
  // the UI from appearing. Download resume, source sync, RPC, redist, etc.
  void runDeferredStartup();
};

const handleCustomDownloadSourceFile = async (filePath: string) => {
  try {
    await importMangoCustomDownloadSource(filePath);
    WindowManager.redirect("settings?tab=downloads");
  } catch (error) {
    logger.error(
      "Error importing Mango custom download source",
      filePath,
      error
    );
  } finally {
    pendingCustomDownloadSourceFile = null;
  }
};
...
app.on("second-instance", (_event, commandLine) => {
  ...
  const customSourceFile = findCustomDownloadSourceFile(commandLine);
  ...
  const isRunDeepLink = deepLink ? new URL(deepLink).host === "run" : false;
  ...
  if (customSourceFile) void handleCustomDownloadSourceFile(customSourceFile);
});
...
app.on("open-file", (event, filePath) => {
  ...
  if (!app.isReady()) {
    pendingCustomDownloadSourceFile = filePath;
    return;
  }

  void handleCustomDownloadSourceFile(filePath);
});
