...
import { launchViaSteam } from "./launch-via-steam";
...
const resolveProtonPathForLaunch = async (
  ...
  const autoDetectedPath = await Umu.findBestProtonPath();
  if (autoDetectedPath) {
    logger.info("Auto-detected Proton path for launch", {
      autoDetectedPath,
    });
    return autoDetectedPath;
  }

  logger.warn("No valid Proton path found for launch");
  return null;
};
...
const launchWindowsBinaryOnLinux = async (
  ...
  // Try launching via Steam Proton if enabled for this game
  if (game?.launchViaSteam === true && game && protonPath) {
    logger.info("Attempting launch via Steam Proton", { objectId });
    try {
      const launched = await launchViaSteam(game, parsedPath, protonPath, {
        useMangohud,
        useGamemode,
        launchOptions,
      });
      ...
      logger.error("launchViaSteam threw unexpectedly", { error });
    }

    logger.warn("Launch via Steam failed, falling back to umu-run");
  }
