...
export default function Notifications() {
  ...
  const fetchBadges = useCallback(async () => {
    try {
      ...
      const badgesResponse = await window.electron.mangoApi.get<Badge[]>(
  ...
  const fetchApiNotifications = useCallback(
    async (
      ...
      try {
        setIsLoading(true);
        const response =
          await window.electron.mangoApi.get<NotificationsResponse>(
    ...
    async (id: string, source: "api" | "local") => {
      try {
        if (source === "api") {
          ...
          await window.electron.mangoApi.patch(
    ...
    async (id: string, source: "api" | "local") => {
      try {
        if (source === "api") {
          ...
          await window.electron.mangoApi.delete(
  ...
  const handleClearAll = useCallback(async () => {
    ...
    try {
      ...
      if (userDetails) {
        await window.electron.mangoApi.delete(`/profile/notifications/all`, {
          needsAuth: true,
        });
      }
