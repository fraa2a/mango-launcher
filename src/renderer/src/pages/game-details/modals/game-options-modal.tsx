...
export function GameOptionsModal({
  ...
  const [launchViaSteam, setLaunchViaSteam] = useState<boolean>(
    game.launchViaSteam ?? game.shop === "steam"
  );
  const [useWined3d, setUseWined3d] = useState<boolean>(
    game.useWined3d === true
  );
  const [useNativeWayland, setUseNativeWayland] = useState<boolean>(
    game.useNativeWayland === true
  );
  ...
  const handleChangeWined3dState = async (value: boolean) => {
    setUseWined3d(value);
    await globalThis.window.electron.toggleGameWined3d(
      game.shop,
      game.objectId,
      value
    );
  ...
  const handleChangeNativeWaylandState = async (value: boolean) => {
    setUseNativeWayland(value);
    await globalThis.window.electron.toggleGameNativeWayland(
      game.shop,
      game.objectId,
      value
    );
            ...
            {selectedCategory === "compatibility" &&
              shouldShowWinePrefixConfiguration && (
                <CompatibilitySettingsSection
                  ...
                  useWined3d={useWined3d}
                  useNativeWayland={useNativeWayland}
                  ...
                  onChangeLaunchViaSteam={handleChangeLaunchViaSteam}
                  onChangeWined3dState={handleChangeWined3dState}
                  onChangeNativeWaylandState={handleChangeNativeWaylandState}
