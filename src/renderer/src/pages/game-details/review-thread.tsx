...
export function ReviewThread({
  ...
  const fetchReplies = async (skip: number, replace: boolean) => {
    ...
    try {
      ...
      const response = await electron.mangoApi.get<
  ...
  const handleVoteReply = async (
    ...
    try {
      const response = await electron.mangoApi.put<
  ...
  const handleDeleteReply = async (answerId: string) => {
    try {
      await electron.mangoApi.delete(`${baseUrl}/${answerId}`);
  ...
  const handlePostReply = async (answerHtml: string) => {
    ...
    try {
      const response = await electron.mangoApi.post<
