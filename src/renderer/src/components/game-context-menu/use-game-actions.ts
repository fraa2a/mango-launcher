...
export function useGameActions(game: LibraryGame) {
  ...
  const handlePlayGame = async () => {
    ...
    if (!canPlay) {
      ...
      globalThis.dispatchEvent(
        new CustomEvent("mango:openRepacks", {
    ...
    if (isClassics) {
      ...
      if (needsModal) {
        ...
        globalThis.dispatchEvent(
          new CustomEvent("mango:openDiscSelection", {
            detail: { objectId: game.objectId },
          })
        );
        return;
    ...
    try {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("mango:game-favorite-toggled", {
  ...
  const handleOpenGameOptions = () => {
    ...
    try {
      window.dispatchEvent(
        new CustomEvent("mango:openGameOptions", {
          detail: { objectId: game.objectId },
        })
      );
    } catch (e) {
  ...
  const handleRemoveFromLibrary = async () => {
    try {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("mango:game-removed-from-library", {
            detail: { shop: game.shop, objectId: game.objectId },
          })
        );
      } catch (e) {
  ...
  const handleRemoveFiles = async () => {
    try {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("mango:game-files-removed", {
            detail: { shop: game.shop, objectId: game.objectId },
          })
        );
      } catch (e) {
